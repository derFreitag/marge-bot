...
def main(cmdline=""):
    def api_mock(gitlab_url, auth_token):
        ...
        api = gitlab_mock.Api(
            gitlab_url=gitlab_url, auth_token=auth_token, initial_state="initial"
        )
    ...
    with mock.patch("marge.bot.Bot", new=DoNothingBot), mock.patch(
        "marge.gitlab.Api", new=api_mock
    ):
